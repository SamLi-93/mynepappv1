...
class MobilebookController extends Controller
{
    ...
    public function actionGetlist(){
        ...
        $conn = Yii::$app->db;
        $sql = "select rid from lc_product_detail where pid=:pid  and type=3";
        $bids = $conn->createCommand($sql,array('pid'=>$pid))->queryAll();
        $bid_arr = ArrayHelper::getColumn($bids, 'rid');
        $bid_arr[] = '0'; 
        $sql = "select id,title  from sms_ebook where id in (".implode(',', $bid_arr).") and is_delete=0";
        $binfo_arr = $conn->createCommand($sql)->queryAll();
        $return_arr = [];
        foreach ($binfo_arr as $k => $v){
        	$rinfo = [];
        	$rinfo['id'] = $v['id'];
        	$rinfo['name'] = $v['title'];
        	$return_arr[] = $rinfo;
    ...
    public function actionGetcontent(){
        ...
        $conn = Yii::$app->db;
        $sql = "select * from sms_ebook_chapter where id=:id";
        $ebinfo = $conn->createCommand($sql,array('id'=>$chapter))->queryOne();
        $chapter_sql = "select * from sms_ebook_chapter where book_id=".$bookid;

        $chapters = $conn->createCommand($chapter_sql)->queryAll();
        $arr = array();
        ...
        $sql = "select download_url from sms_ebook where id=".$bookid;
       	$book_url = $conn->createCommand($sql,array('id'=>$chapter))->queryColumn();
       	$epub_url = Yii::$app->params['epub_url'];
       	$dpath_arr = explode('.', $book_url[0]);
       	$dpath = $dpath_arr[0];
        $return_arr['title'] = $ebinfo['title'];
        $return_arr['content'] = strip_tags(file_get_contents($epub_url.$dpath.'/OEBPS/'.$ebinfo['filename']));
        $return_arr['chapter'] = $chapter_list;
        $return_info = json_encode(array('responseData'=>$return_arr)); 
        $callback = $request->get ( 'callback' );
        echo $callback . "(" . $return_info . ")";
        exit ();
    ...
    public function actionGetmenucontent(){
        ...
        switch ($chapter) {
            case '1':
                $return_arr['title'] = "第一章 朝气蓬勃";
                $return_arr['content'] = "石村，位于苍莽山脉中，四周高峰大壑，茫茫群山巍峨。
    ...
    “祭灵，请保佑族人，让孩子们打到肥美的猎物，平安归来。我们将以虔诚的心，世代祭祀与供养你。”";
                break;
            case '2':
                $return_arr['title'] = "第二章 骨文";
                $return_arr['content'] = "在族长与一些老人的祷告下，所有青壮年都露出郑重之色，进行礼拜。◎ ◎而不少妇孺也都赶了过来，默默祈祷，祈求去狩猎的亲人可以无恙的回来。
    ...
    族长石云峰带领众人走向柳木，抬着数十具兽尸来到近前后，将所有血迹斑斑的凶兽都放在了石台上，显然这是一个大型祭台。";
                break;
            case '3':
                $return_arr['title'] = "第三章 祭灵";
                $return_arr['content'] = "祭台以巨石砌成，很开阔，紧邻折断的老柳树而建，此时上面堆积满了猛兽，如同一座小山似的。※ ※
